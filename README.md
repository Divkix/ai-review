...
**Alpha.** Callers pin an exact release tag (currently `@v0.4.9`), not a floating major — the API is still changing. Cutting a release bumps every internal `ref:`/`@vX` pin to the new tag and tags the commit that carries them; the [`Release` workflow](.github/workflows/release.yml) automates this two ways:
