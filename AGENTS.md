...
- `scripts/lib/reconcile.sh`, `scripts/lib/sarif.sh`, `scripts/lib/context.sh`, `scripts/lib/scope.sh` — pure, unit-tested logic the workflows `source` (single source of truth, no copy drift): baseline/state/thread reconciliation, SARIF merge, cross-file impact-map building, and per-repo scope config parsing/path matching.
...
`review.yml` is a five-job pipeline: **gate** (resolve PR/SHA, skip drafts, fetch `.ai-review.yml` from base branch, config-driven size guard, emit `ignore_patterns`) → **static** (opengrep/gitleaks/osv → SARIF uploads → `findings.json` filtered by config ignore patterns) and **context** (ripgrep cross-file impact map with ignore-pattern filtering → `context.md`) in parallel → **llm-review** (read-only; runs the opencode CLI with the prompt + artifacts + `IGNORE_PATHSPECS` env) → **finalize** (`contents:write`; dismisses superseded reviews, resolves threads). `commands.yml` routes `/review`, `/plan`, `/oc` comments into the same review. State lives in one sticky status comment carrying an embedded `<!-- ai-review:state … -->` marker; only the bot-authored marker is trusted, and `reconcile.sh` derives the effective baseline, prior verdict, and open threads from it.
